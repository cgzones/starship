...
pub fn get_configuration(context: &Context) -> toml::Table {
    let starship_config = StarshipConfig::initialize(context.get_config_path_os().as_ref());
...
pub fn get_configuration_edit(context: &Context) -> DocumentMut {
    ...
    let toml_content = StarshipConfig::read_config_content_as_str(config_file_path.as_ref());
